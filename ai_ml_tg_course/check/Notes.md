...
# Day 14 - Nov 2, 2025 - Statistices 2
...
- **Percentile** → comparison with a group

-



### Things to remember while calculating these
...
where:
- PL = Percentile Location
- p = desired percentile (e.g., 75 for the 75th percentile)
- N = number of observations in the dataset

**Example:**

Find the 75th percentile score from the below data, so p = 75
...
PL = 75                3            33
    ---- X (10 + 1) = ---- X 11 =  ----- = 8.25 ( this is locaion)
    100                4             4
...
import numpy as np 
data = np.array([78, 82, 84, 91, 94, 96, 98, 99])
...
Now i would like to find 88 number rank

= 3 + 0.5 x 1 (88 comes only one time hence 1)
  --------------------------------------------               
    10 (total number we have in data set)
...
| Method      | Description                                                     | Formula Type                        | Example Result (your data)   |
|-------------|-----------------------------------------------------------------|-------------------------------------|------------------------------|
| `linear` *(default)* | Linear interpolation between closest ranks              | (i = (p/100)(n-1))                  | **95.5**                     |
| `lower`     | Always choose the lower nearest rank                            | Stepwise                            | 94                           |
| `higher`    | Always choose the higher nearest rank                           | Stepwise                            | 96                           |
| `midpoint`  | Average of lower and higher                                     | Stepwise average                    | 95                           |
| `nearest`   | Nearest observation                                             | Round to nearest                    | 96                           |
| `weibull`   | Uses ((i = p/100 × (n+1))) — similar to Excel’s **PERCENTILE.INC** | “N+1” method                        | **96.5**                     |
| `hazen`     | Uses ((i = p/100 × (n+0.5)))                                    | Intermediate between Weibull & linear | 96.125 (approx)             |


lst = [78, 82, 84, 88, 91, 93, 94, 96, 98, 99]

import numpy as np
data = [78, 82, 84, 88, 91, 93, 94, 96, 98, 99]
percentile_75 = np.percentile(data, 75, method='weibull')
print(percentile_75)
...
how different methods in numpy.percentile() produce different results for the 75th percentile, matching the table in your earlier image. The 'weibull' method matches Excel's PERCENTILE.INC and gives 96.5, while the default ('linear') gives 95.5.
========================
...
Minium  LowerQuartile  Medium  Upperquartile      Maximum
-------------------------------------------------------
25%                     



### Five Number Summary Visualization and Interquartile Range (IQR)

- **Minimum**, **Lower Quartile (Q1, 25%)**, **Median (Q2, 50%)**, **Upper Quartile (Q3, 75%)**, **Maximum**  
  - Each quartile represents 25% of the dataset.
...
### # Boxplots
- histrogram can't work on single data set. - [6, 12, 478, 29]

## What is a boxplot

A box plot, also known as a box-and-whisker plot, is a graphical representation of a dataset that shows the distribution of the data. The box plot displays data, including the minimum and maximum values, the first quartile (Q1), the median (Q2), and the third quartile (Q3).

### How to create a boxplot with example

Data:  
6, 213, 241, 260, 281, 290, 314, 321, 350, 1500

**General Formula:**
- **Q1 position** = (n + 1) × 0.25 = 2.75 (between 2nd and 3rd values)
- **Q2 position** = (n + 1) × 0.5 = 5.5 (between 5th and 6th values)
- **Q3 position** = (n + 1) × 0.75 = 8.25 (between 8th and 9th values)

*Note:* Position counting starts at 1, not 0 (unlike Python array indices).

---

- **Lower Whisker:**
  - Extends to the **smallest data point** that is ≥ (Q1 - 1.5 × IQR)
- **Upper Whisker:**
  - Extends to the **largest data point** that is ≤ (Q3 + 1.5 × IQR)

Where **IQR (Interquartile Range) = Q3 - Q1**

IQR = 328.75 - 239 = 94.75

Min = q1 - 1.5  (IQR) = 234 - (1.5  X 94.75) = 91.876


Max = Q3 + (1.5 X IQR)  =  3  + (1.5 X 94.75)


beneifts of boxplot

example of bobxplot in python:
import matplotlib.pyplot as plt
